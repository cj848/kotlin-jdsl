...
object Paths {
    ...
    fun <T : Any, V> path(
        entity: Entity<T>,
        property: KProperty1<in T, @Exact V>,
    ): Path<V & Any> = JpqlEntityProperty(entity, property)
    ...
    fun <T : Any, V> path(
        entity: Entity<T>,
        getter: KFunction1<T, @Exact V>,
    ): Path<V & Any> = JpqlEntityProperty(entity, getter)
    ...
    fun <T : Any, V> path(
        path: Path<T>,
        property: KProperty1<in T, @Exact V>,
    ): Path<V & Any> = JpqlPathProperty(path, property)
    ...
    fun <T : Any, V> path(
        path: Path<T>,
        getter: KFunction1<T, @Exact V>,
    ): Path<V & Any> = JpqlPathProperty(path, getter)
    ...
    fun <T : Any, S : T> treat(
        path: Path<T>,
        type: KClass<S>,
    ): Path<S> = JpqlPathTreat(path, type)
}
